...
export class BookARoomComponent implements OnInit {
  ...
  submitBooking(): void {
    ...
    const bookingModel: Booking = {
     ...
     // Call the backend to book the room
     this.bookingService.createBooking(bookingModel).subscribe(
      (response) => {
        this.bookingSuccess = true;
        this.bookingReference = response.bookingReference; // Get the booking reference from the backend
      ...
      (error) => {
        if (error.error instanceof ProgressEvent) {
          // Handle network or server unreachable error
          this.errorMessage = 'Network error or server is unreachable.';
        } else if (error.status === 500) {
          // Handle server-side 500 error
